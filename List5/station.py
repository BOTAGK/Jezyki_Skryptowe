...
class Station:
    ...
    def __init__(self, path: Path, target_code: str) -> None:
        self.code = target_code
        self._parse_data(path, target_code)

   

    def _parse_data(self, row: Dict) -> None:
        ...
        lat_str = row.get(StationColumn.WGS84_N, '').replace(',', '.')
        lon_str = row.get(StationColumn.WGS84_E, '').replace(',', '.')
