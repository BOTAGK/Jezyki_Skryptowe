...
class Config:
    ...
    DEFAULT_UNIT: str = "ng/m3"
    UNKNOWN_VALUE: str = "unknown"
    HEADER_LINES_TO_SKIP_FOR_DATA: int = 6
    FILENAME_REGEX: re.Pattern[str] = re.compile(r"^(\d{4})_(.+?)_(\d+[a-zA-Z]+)(?:_\d+)?\.csv$")
...
class Measurements:
    ...
    def _build_index(self) -> None:
        ...
        for file_path in self.directory.glob(Config.CSV_GLOB):
            metadata: FileMetadata = self._process_single_file_metadata(file_path)
    ...
    def _process_single_file_metadata(self, file_path: Path) -> FileMetadata:
        ...
        stations: List[str] = self._extract_stations_from_header(file_path)
    ...
    def _parse_filename(self, file_path: Path) -> Tuple[str, str, str]:
        ...
        match: Optional[re.Match[str]] = Config.FILENAME_REGEX.fullmatch(file_path.name)
    ...
    def _load_data_if_needed(self, metadata: FileMetadata) -> None:
        ...
        values_dict: Dict[str, List[Optional[float]]]

        dates, values_dict = self._parse_measurements_file(metadata)
        self._instantiate_time_series(metadata, dates, values_dict)

    def _parse_measurements_file(
            self, metadata: FileMetadata
        ) -> Tuple[List[datetime.datetime], Dict[str, List[Optional[float]]]]:
        """Function responsible for opening and reading the actual measurement rows."""
        dates: List[datetime.datetime] = []
        values_dict: Dict[str, List[Optional[float]]] = {st_code: [] for st_code in metadata.stations}
    ...
    def _process_measurement_row(
        ...
        date_obj: datetime.datetime = self._parse_date(row[0])
        dates.append(date_obj)

        for i, st_code in enumerate(stations):
            val: Optional[float] = self._parse_value(row, index=i+1)
    ...
    def _parse_value(self, row: List[str], index: int) -> Optional[float]:
        ...
        val_str: str = row[index].strip() if index < len(row) else ""
        if not val_str:
            return None
        try:
    ...
    def detect_all_anomalies(self, validators: list[SeriesValidator], preload: bool = False) -> dict[str, list[str]]:
        ...
        for metadata in self._files_index:
            ...
            for series in metadata.loaded_series.values():
                dict_key: str = f'{series.station_name}_{metadata.frequency}'
